...
class VolumeEstimator:
    def __init__(self, orthogonal_lines_dict: dict, h: float, k_conv_factor: Optional[float]):
        ...
        Parameters:
            orthogonal_lines_dict: Dictionary with keys being the name of the body part and values being lists of lines.
    ...
    def calculate_volume_in_mm_3(self, round_to) -> tuple[float, dict]:
        """
        Calculate the estimated volume of the structure for each body part separately and the total volume.
        Units: mm^3
        """
        total_volume = 0
        ...
        for body_part, lines in self.orthogonal_lines_dict.items():
            for i in range(1, len(lines)):
                ...
                if len(elements_per_line_i) < 2 or len(elements_per_line_i_1) < 2:
                    print(f'Both lines must contain at least two points. Got {len(elements_per_line_i_1)} and {len(elements_per_line_i)} points for segment: {body_part}')
                    print("Skipping...")
                    continue
