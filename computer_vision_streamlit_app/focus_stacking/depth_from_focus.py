...
def main(base_path):
    ...
    # Set paths
    img_path = os.path.join(base_path, 'align')
    focus_save_path = os.path.join(base_path, 'focus_stack')
    depth_save_path = os.path.join(base_path, 'depth_map')
    all_focus_save_path = os.path.join(base_path, 'all_in_focus')
    graph_save_path = os.path.join(base_path, 'graph_cut')
    wmf_save_path = os.path.join(base_path, 'wmf')
    save_as = 'output.jpg'
    
    img_list = read_images_from_path(img_path)
    if not img_list:
        raise ValueError("The list of images is empty. Please check the path.")
    ...
    for i, aligned_img in enumerate(img_list):
        ...
        laplacian_img = focus_stack(aligned_img, gaussian_size=gaussian_size, laplacian_size=laplacian_size)
    ...
    depth_map, all_in_focus_img = all_in_focus(img_list, cost_volume, kernel_size=kernel_size, gaussian_size=gaussian_size)
...
if __name__ == '__main__':
    ...
    base_path = '/Users/joshuaalbiez/Documents/python/computer_vision_streamlit_app/data/focus_stacking'
    main(base_path)