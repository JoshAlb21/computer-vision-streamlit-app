...
def compute_cogs(masks: list, all_cls: torch.Tensor, label: dict) -> dict:
    ...
    if 'dog' in label.values() and 'bicycle' in label.values():
        head_id = [k for k, v in label.items() if v == 'dog'][0]
        thorax_id = [k for k, v in label.items() if v == 'bicycle'][0]
        try:
    ...
    if 'bicycle' in label.values() and 'umbrella' in label.values():
        thorax_id = [k for k, v in label.items() if v == 'bicycle'][0]
        abdomen_id = [k for k, v in label.items() if v == 'umbrella'][0]
        try:
