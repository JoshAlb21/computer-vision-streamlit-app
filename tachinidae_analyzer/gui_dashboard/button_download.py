...
def button_download_all(all_data:Dict[str, BytesIO]):
    ...
    data_to_keep = {}
    data_to_keep['cog_body_parts.csv'] = all_data['cog_body_parts.csv']
    ...
    with st.sidebar:
        st.sidebar.write("CoG for each body part are downloaded by default. Select further files to download.")
        st.download_button(
            "Download csv", 
